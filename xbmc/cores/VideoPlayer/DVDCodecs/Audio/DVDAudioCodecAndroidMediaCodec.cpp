...
bool CDVDAudioCodecAndroidMediaCodec::Open(CDVDStreamInfo &hints, CDVDCodecOptions &options)
{
  ...
  if(m_hints.extrasize > 6 && strncmp((char*)m_hints.extradata,"CRYPTO", 6) == 0)
  {
    ...
    CLog::Log(LOGDEBUG, "CDVDAudioCodecAndroidMediaCodec:: CRYPTO size(%d), EXTRA size(%d)", cryptoSize, m_hints.extrasize);
    //if (!m_hints.extrasize) m_hints.extrasize = 4; /* NASTY HACK */
...
int CDVDAudioCodecAndroidMediaCodec::Decode(uint8_t* pData, int iSize, double dts, double pts)
{
  int rtn = 0;
  if (g_advancedSettings.CanLogComponent(LOGAUDIO))
    CLog::Log(LOGDEBUG, "CDVDAudioCodecAndroidMediaCodec::Decode dts:%0.4lf pts:%0.4lf size(%d)", dts, pts, iSize);
  ...
  if (pData)
  {
    ...
    else if (index >= 0)
    {
      ...
      if (pData && iSize && !m_cryptoData.empty())
      {
        ...
        {
          numSubsamples = 1;
          clearBytes.push_back(0);
          cipherBytes.push_back(iSize);
        }
        cryptoInfo->set(numSubsamples, clearBytes, cipherBytes, kid, iv, CJNIMediaCodec::CRYPTO_MODE_AES_CTR);
        CLog::Log(LOGDEBUG, "CDVDVideoCodecAndroidMediaCodec::Decode size: %d, NumSubsamples: %d", (unsigned int) iSize, numSubsamples);
      }
      ...
      if (xbmc_jnienv()->ExceptionCheck())
      {
        CLog::Log(LOGERROR, "CDVDMediaCodecInfo::getInputBuffers ExceptionCheck");
...
bool CDVDAudioCodecAndroidMediaCodec::ConfigureMediaCodec(void)
{
  ...
  {
    CLog::Log(LOGDEBUG, "MediaCodecAudio::ConfigureMediaCodec: Parsing Crypto (size: %u)", (unsigned int)m_cryptoData.size());
    ...
    if (xbmc_jnienv()->ExceptionCheck())
    {
      CLog::Log(LOGERROR, "MediaCryptoAudio::ExceptionCheck: <init>");
      xbmc_jnienv()->ExceptionDescribe();
      xbmc_jnienv()->ExceptionClear();
      return false;
    }
